...
int main(int argc, char *argv[])
{
    ...
    {
        if (pos_symbol.second != '*')
        {
            // 非 * 字符
            continue;
        }
        else if ((number_map.find(pos_symbol.first) != number_map.end()) &&
                 (number_map.find(pos_symbol.first)->second.size() == 2))
        {
            //! WARNING: 这里假设每个数字字符串有且仅有一个相邻的非dot字符
            // * 字符 且对应有两个相邻的数字字符串
            result += number_map.find(pos_symbol.first)->second[0] *
                      number_map.find(pos_symbol.first)->second[1];
        }
    }
    ...
    std::cout << "Result: " << result << std::endl; // right answer: 87449461
